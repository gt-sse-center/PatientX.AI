            ...
            for line in response.iter_lines():
                if line:
                    try:
                        ...
                        if 'message' in data:
                            print(data['message']['content'], end='')  # print when api/generate is used
                        if 'response' in data:
                            print(data['response'], end='')  # print when api/chat is used
                    except json.JSONDecodeError:
                        print(f"Failed to decode JSON: {line}")
        else:
            print(f"Failed to retrieve response. Status Code: {response.status_code} Response: {response.text}")

def load_and_preprocess_data(filepath):
    """ Load and preprocess CSV data. """
    data = pd.read_csv(filepath)
    # Assuming there's a column named 'text' that contains the posts
    texts = data['post_message'].dropna().tolist()  # Drop missing values and convert to list
    return ' '.join(texts)  # Join all texts into a single string

def summarize_text(text, model_url, chunk_size=1024):
